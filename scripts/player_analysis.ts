...
const factionAdjustments: FactionAdjustments = {
  'ISDF': {
    'Scion': 0.5215,
  },
  'Hadean': {
...
function getFactionAdjustment(faction1: string, faction2: string): [number, number] {
...
function readElos(filePath: string, eloType: 'commander' | 'thug') {
    ...
    for (const player in elos) {
        ...
        if (typeof elos[player] === 'number') {
            ...
            const data = elos[player] as { elo: number, wins: number, losses: number };
            if (eloType === 'commander') {
...
readElos('../data/updated_commander_elos.js', 'commander');
readElos('../data/updated_thug_elos.js', 'thug');

const rawGameData = fs.readFileSync('../data/games.ts', 'utf-8');
const gamesJson = rawGameData.substring(rawGameData.indexOf('[')).replace('];', ']');
const games: Game[] = JSON.parse(gamesJson);



function getPlayerElo(playerName: string): PlayerElos {
  if (!playerName) {
...
function calculateTeamStrength(commander: string, thugs: string[], faction1: string, faction2: string, isTeam1: boolean): number {
